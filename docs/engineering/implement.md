...
Dispatch goes to a named agent role: `implementer` for ticket work, `reviewer` for the review axes. [setup-matt-pocock-skills](https://aihero.dev/skills-setup-matt-pocock-skills) installs the role definitions into the project — `.claude/agents/` (Claude Code), `.codex/agents/` (Codex), `.opencode/agents/` (OpenCode) — with model and effort defaults living in the role definition rather than in skill prose. The copies are per-project on purpose: each project can tune its own roles, and setup never overwrites an existing definition. On a harness where the role doesn't resolve, dispatch falls back to the default sub-agent mechanism and the run ledger records the fallback.
