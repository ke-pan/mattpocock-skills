...
Dispatch each reviewer from its complete review brief alone — never from forked conversation history. The `reviewer` role definition carries the model and effort; tune them per project in the role file. Apply a per-spawn override only when the user, repository, or calling workflow explicitly requires one, and record any override actually applied — never report a runtime setting that was not. Direct attention instead of effort: where the spec declares stateful boundaries or an evidence contract, name them in the brief so the reviewer concentrates there. Record the role (or named fallback) and the reviewer handle; if a reviewer inherited conversation history or its role went unrecorded, interrupt and relaunch before accepting a verdict.
